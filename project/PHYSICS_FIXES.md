...
- Final Geometry custom-script cleanup: a scripted grounded rotation reset is not treated as horizontal movement on flat ground, so `rotation = 0/90/...` cannot manufacture forward/backward travel when `velocityX` is untouched.
- Regression added for the exact custom script pattern: gravity + airborne rotation + grounded rotation reset + jump, verifying no unintended X drift and no landing vibration.
